...
const LoginModal: React.FC<LoginModalProps> = ({ onLogin, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isProduction) {
      setTimeout(() => {
        if (userNumber === '1234' || userNumber === '882941') {
          ...
          setLoading(false);
          setStatus('error');
          setMessage('Modo Demonstração: use o ID 1234 para testar.');
          triggerShake();
        }
      }, 1200);
      return;
    ...
    try {
      ...
      setLoading(false);
      setStatus('error');
      setMessage('ID de usuário não localizado ou assinatura expirada.');
      triggerShake();
    }
  };
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-10 custom-scrollbar">
              ...
              <div className="flex justify-between items-center ml-1">
                ...
                {status === 'error' && (
                  <span className="text-[10px] text-red-500 font-bold flex items-center gap-1 animate-in slide-in-from-right-2">
                    <AlertCircle size={10} /> Não encontrado
                  </span>
                )}
              </div>
          ...
          {!isProduction && (
            <div className="mt-4 p-3 bg-blue-600/10 border border-blue-500/20 rounded-xl">
              <p className="text-[10px] text-blue-400 text-center font-bold uppercase tracking-widest">Dica: Use ID 1234 (Modo Demo)</p>
            </div>
          )}
          ...
          <div className="mt-8 sm:mt-12 pt-6 sm:pt-8 border-t border-white/5 text-center">
            <p className="text-[11px] sm:text-xs text-gray-500 leading-relaxed px-2">
              Esqueceu seu ID ou precisa de um novo? Fale com nosso suporte.
            </p>
            <a 
              href="https://wa.me/5598982804577?text=Olá!+Preciso+de+ajuda+com+meu+ID+de+acesso+Guito+Plus."
              target="_blank"
